...
from src.managers.dashboard import DashboardManager
...
async def get_dashboard(
    current_user: CurrentUserResponse = Depends(require_permission('dashboard:read')),
    db: Session = Depends(get_database),
) -> DashboardResponse:
    return DashboardManager.get_dashboard(db, current_user_email=current_user.email)


@router.get('/tasks', response_model=DashboardTaskListResponse)
async def list_tasks(
    current_user: CurrentUserResponse = Depends(require_permission('dashboard:manage')),
    db: Session = Depends(get_database),
) -> DashboardTaskListResponse:
    return DashboardManager.list_tasks(db)
...
async def create_task(
    ...
    try:
        return DashboardManager.create_task(db, payload)
    except ValueError as exc:
        detail = str(exc)
        status_code = (
            status.HTTP_404_NOT_FOUND
            if detail in {'Assigned user not found', 'Client not found', 'Quote not found'}
...
async def update_task(
    ...
    try:
        return DashboardManager.update_task(db, task_id, payload)
    except ValueError as exc:
        detail = str(exc)
        status_code = (
            status.HTTP_404_NOT_FOUND
            if detail in {'Task not found', 'Assigned user not found', 'Client not found', 'Quote not found'}
            else status.HTTP_400_BAD_REQUEST
        )
        raise HTTPException(status_code=status_code, detail=detail) from exc
...
async def delete_task(
    ...
    try:
        DashboardManager.delete_task(db, task_id)
    except ValueError as exc:
...
async def list_events(
    ...
    return DashboardManager.list_events(db)
...
async def create_event(
    ...
    try:
        return DashboardManager.create_event(db, payload)
    except ValueError as exc:
        detail = str(exc)
        status_code = (
...
async def update_event(
    ...
    try:
        return DashboardManager.update_event(db, event_id, payload)
    except ValueError as exc:
        detail = str(exc)
        status_code = (
            status.HTTP_404_NOT_FOUND if detail in {'Event not found', 'Assigned user not found'} else status.HTTP_400_BAD_REQUEST
...
async def delete_event(
    ...
    try:
        DashboardManager.delete_event(db, event_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    return Response(status_code=status.HTTP_204_NO_CONTENT)
...
async def list_notifications(
    ...
    return DashboardManager.list_notifications(db)
...
async def create_notification(
    ...
    return DashboardManager.create_notification(db, payload)
...
async def update_notification(
    ...
    try:
        return DashboardManager.update_notification(db, notification_id, payload)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
...
async def delete_notification(
    ...
    try:
        DashboardManager.delete_notification(db, notification_id)
    except ValueError as exc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(exc)) from exc
    return Response(status_code=status.HTTP_204_NO_CONTENT)
...
async def list_message_contacts(
    current_user: CurrentUserResponse = Depends(require_permission('messages:read')),
    db: Session = Depends(get_database),
) -> DashboardContactListResponse:
    return DashboardManager.list_message_contacts(db, current_user_email=current_user.email)
...
async def list_messages(
    ...
    return DashboardManager.list_messages(db, current_user_email=current_user.email)
...
async def send_message(
    ...
    current_user: CurrentUserResponse = Depends(require_permission('messages:write')),
    db: Session = Depends(get_database),
) -> DashboardNotificationRead:
    try:
        return DashboardManager.send_message(db, sender_email=current_user.email, payload=payload)
    except ValueError as exc:
        detail = str(exc)
...
async def mark_message_read(
    ...
    try:
        return DashboardManager.mark_message_read(
            db,
