...
from src.db.repositories.dashboard import DashboardRepository
...
class DashboardManager:
    ...
    def get_dashboard(
        ...
        *,
        ...
        current_user = UsersManager.get_by_email(db, current_user_email)
        ...
        task_rows = db.execute(
            select(DashboardTask, User.full_name, User.email, Client.company_name, Quote.reference)
            .outerjoin(User, DashboardTask.assigned_user_id == User.id)
            .outerjoin(Client, DashboardTask.client_id == Client.id)
            .outerjoin(Quote, DashboardTask.quote_id == Quote.id)
        ...
        notifications_rows = DashboardManager._notification_rows_for_user(db, current_user.id)[:5]
        ...
        return DashboardResponse(
            ...
            tasks=[
                DashboardManager._serialize_task(
                    task,
                    assigned_user_name=full_name or email,
                    client_name=client_name,
                    quote_reference=quote_reference,
                )
                for task, full_name, email, client_name, quote_reference in task_rows
            ],
            events=[
                DashboardManager._serialize_event(
                    event,
                    assigned_user_name=full_name or email,
                )
            ...
            notifications=[
                DashboardManager._serialize_notification(
                    item,
    ...
    def create_task(db: Session, payload: DashboardTaskCreate) -> DashboardTaskRead:
        DashboardManager._validate_task_links(
            db,
        ...
        DashboardRepository.add_task(db, task)
        db.commit()
        db.refresh(task)
        return DashboardManager._read_task(db, task.id)

    @staticmethod
    def update_task(db: Session, task_id: int, payload: DashboardTaskUpdate) -> DashboardTaskRead:
        task = DashboardRepository.get_task(db, task_id)
        if task is None:
    ...
    def delete_task(db: Session, task_id: int) -> None:
        ...
        if task is None:
            raise ValueError('Task not found')
        DashboardRepository.delete_task(db, task)
        db.commit()
    ...
    @staticmethod
    def _read_task(db: Session, task_id: int) -> DashboardTaskRead:
        row = db.execute(
            select(DashboardTask, User.full_name, User.email, Client.company_name, Quote.reference)
            .outerjoin(User, DashboardTask.assigned_user_id == User.id)
            .outerjoin(Client, DashboardTask.client_id == Client.id)
            .outerjoin(Quote, DashboardTask.quote_id == Quote.id)
            .where(DashboardTask.id == task_id)
        ).one()
        task, full_name, email, client_name, quote_reference = row
        return DashboardManager._serialize_task(
            task,
            assigned_user_name=full_name or email,
            client_name=client_name,
            quote_reference=quote_reference,
    ...
    def create_event(db: Session, payload: DashboardEventCreate) -> DashboardEventRead:
        ...
        DashboardRepository.add_event(db, event)
        db.commit()
        db.refresh(event)
        ...
        return DashboardManager._serialize_event(event, assigned_user_name=assigned_user_name)

    @staticmethod
    def update_event(db: Session, event_id: int, payload: DashboardEventUpdate) -> DashboardEventRead:
        event = DashboardRepository.get_event(db, event_id)
        if event is None:
    ...
    def delete_event(db: Session, event_id: int) -> None:
        ...
        if event is None:
            raise ValueError('Event not found')
        DashboardRepository.delete_event(db, event)
        db.commit()
    ...
    def create_notification(db: Session, payload: DashboardNotificationCreate) -> DashboardNotificationRead:
        ...
        DashboardRepository.add_notification(db, item)
        db.commit()
        db.refresh(item)
        sender_email = None
        ...
        return DashboardManager._serialize_notification(
            item,
            sender_email=sender_email,
            recipient_name=recipient_name,
            recipient_email=recipient_email,
    ...
    def update_notification(
        ...
        item = DashboardRepository.get_notification(db, notification_id)
        if item is None:
    ...
    def delete_notification(db: Session, notification_id: int) -> None:
        ...
        if item is None:
            raise ValueError('Notification not found')
        DashboardRepository.delete_notification(db, item)
        db.commit()
    ...
    def list_messages(
        ...
        *,
        ...
        rows = DashboardManager._notification_rows_for_user(db, current_user.id)
    ...
    def send_message(
        ...
        *,
        ...
        sender_user = UsersManager.get_by_email(db, sender_email)
        ...
        DashboardRepository.add_notification(db, message)
        db.commit()
        db.refresh(message)
        return DashboardManager._serialize_notification(
            message,
    ...
    def mark_message_read(
        ...
        *,
        ...
        item = DashboardRepository.get_notification(db, message_id)
